...
describe("POST /api/collections", () => {
  it("returns 401 when unauthenticated", async () => {
    ...
    const res = await POST(
      makeReq("POST", "/api/collections", { collection_name: "Work" }),
    );
    expect(res.status).toBe(401);
  });
  ...
  it("returns 500 on database error", async () => {
    ...
    const res = await POST(
      makeReq("POST", "/api/collections", { collection_name: "X" }),
    );
    expect(res.status).toBe(500);
  });
});
...
describe("PATCH /api/collections", () => {
  ...
  it("returns 400 when id is missing", async () => {
    authOk();
    const res = await PATCH(
      makeReq("PATCH", "/api/collections", { collection_name: "New" }),
    );
...
describe("DELETE /api/collections", () => {
  it("returns 401 when unauthenticated", async () => {
    ...
    const res = await DELETE(
      makeReq("DELETE", "/api/collections", { ids: ["c1"] }),
    );
    expect(res.status).toBe(401);
  });
  ...
  it("returns 400 when ids array is empty", async () => {
    authOk();
    const res = await DELETE(
      makeReq("DELETE", "/api/collections", { ids: [] }),
    );
    expect(res.status).toBe(400);
  });
  ...
  it("returns 404 when no collections owned by user", async () => {
    authOk();
    dbResult.data = []; // no owned collections
    dbResult.error = null;
    const res = await DELETE(
      makeReq("DELETE", "/api/collections", { ids: ["c1"] }),
    );
  ...
  it("returns 200 on successful deletion", async () => {
    authOk();
    dbResult.data = [{ id: "c1" }]; // owned collection found
    dbResult.error = null;
    const res = await DELETE(
      makeReq("DELETE", "/api/collections", { ids: ["c1"] }),
    );
