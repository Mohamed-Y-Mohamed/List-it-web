 ...
 *
 * Unit tests for GET | POST | PATCH | DELETE /api/tasks
 */
...
describe("GET /api/tasks", () => {
  ...
  it("returns 200 with tasks list", async () => {
    authOk();
    dbResult.data = [{ id: "t1", text: "Buy milk" }];
    const res = await GET(makeReq("GET", "/api/tasks"));
  ...
  it("filters by list_id", async () => {
    authOk();
    dbResult.data = [];
    const res = await GET(makeReq("GET", "/api/tasks?list_id=l1"));
    expect(res.status).toBe(200);
    expect(mockChain.eq).toHaveBeenCalledWith("list_id", "l1");
  });

  it("filters by is_completed", async () => {
    authOk();
    dbResult.data = [];
    const res = await GET(makeReq("GET", "/api/tasks?is_completed=true"));
    expect(res.status).toBe(200);
    expect(mockChain.eq).toHaveBeenCalledWith("is_completed", true);
  });

  it("filters by is_deleted", async () => {
    authOk();
    dbResult.data = [];
    const res = await GET(makeReq("GET", "/api/tasks?is_deleted=false"));
    expect(res.status).toBe(200);
  ...
  it("filters by is_pinned", async () => {
    authOk();
    dbResult.data = [];
    const res = await GET(makeReq("GET", "/api/tasks?is_pinned=true"));
    expect(res.status).toBe(200);
...
describe("POST /api/tasks", () => {
  it("returns 401 when unauthenticated", async () => {
    authFail();
    const res = await POST(makeReq("POST", "/api/tasks", { text: "Buy milk" }));
    expect(res.status).toBe(401);
  });

  it("returns 201 with the new task", async () => {
    authOk();
    dbResult.data = { id: "t1", text: "Buy milk" };
    const res = await POST(makeReq("POST", "/api/tasks", { text: "Buy milk" }));
  ...
  it("injects user_id from session", async () => {
    authOk();
    dbResult.data = { id: "t1", text: "X", user_id: "user-1" };
    await POST(makeReq("POST", "/api/tasks", { text: "X" }));
  ...
  it("returns 500 on database error", async () => {
    ...
    const res = await POST(makeReq("POST", "/api/tasks", { text: "X" }));
    expect(res.status).toBe(500);
  });
});
...
describe("PATCH /api/tasks", () => {
  ...
  it("returns 400 when id is missing", async () => {
    authOk();
    const res = await PATCH(makeReq("PATCH", "/api/tasks", { text: "New" }));
...
describe("DELETE /api/tasks", () => {
  it("returns 401 when unauthenticated", async () => {
    authFail();
    const res = await DELETE(makeReq("DELETE", "/api/tasks", { id: "t1" }));
    expect(res.status).toBe(401);
  });

  it("returns 400 when id is missing", async () => {
    authOk();
    const res = await DELETE(makeReq("DELETE", "/api/tasks", {}));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toMatch(/id/i);
  });

  it("soft-deletes a task (sets is_deleted=true)", async () => {
    authOk();
    dbResult.error = null;
    const res = await DELETE(makeReq("DELETE", "/api/tasks", { id: "t1" }));
    expect(res.status).toBe(200);
  ...
  it("hard-deletes a task when hard=true", async () => {
    authOk();
    dbResult.error = null;
    const res = await DELETE(makeReq("DELETE", "/api/tasks", { id: "t1", hard: true }));
    expect(res.status).toBe(200);
