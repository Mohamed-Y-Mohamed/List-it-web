          ...
          <div className="flex justify-between items-center">
            <div>
              <div className="flex items-center mb-2">
                <Calendar
                  className={`h-7 w-7 mr-3 ${isDark ? "text-purple-400" : "text-purple-500"}`}
                />
                <h1
        ...
        {!isLoading && tasks.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <StatsCard
              ...
              value={totalTasks > 0 ? Math.min(100, totalTasks * 20) : 0}
              icon={TrendingUp}
              color="bg-blue-500"
              isDark={isDark}
              suffix="%"
              description="Preparation level"
              isLoading={isLoading}
            />
          </div>
        )}
          ...
          >
            <div className="flex flex-col items-center justify-center space-y-4">
              <div className="relative">
                <RefreshCw
                  className={`h-10 w-10 animate-spin ${isDark ? "text-purple-400" : "text-purple-500"}`}
                />
                <div
                  className={`absolute inset-0 animate-pulse ${isDark ? "bg-purple-400" : "bg-purple-500"} rounded-full opacity-20`}
                ></div>
              </div>
              <p className="text-lg font-medium">
                Loading tomorrow&aposs tasks...
              </p>
            </div>
          </motion.div>
          ...
          >
            <AnimatePresence>
              {sortedTasks.map((task, index) => (
                ...
                >
                  <div className="absolute -top-2 -right-2 px-3 py-1 rounded-full text-xs font-bold bg-purple-500 text-white shadow-lg z-10">
                  ...
                  <TodayTaskCard
                    ...
                    className="border-l-4 border-purple-500 hover:-translate-y-1 hover:shadow-lg transition-all duration-200"
                  />
                </motion.div>
              ))}
            </AnimatePresence>
